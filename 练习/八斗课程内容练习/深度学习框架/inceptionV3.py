...
from keras.layers import Activation, Dense, Input, BatchNormalization, Conv2D, MaxPooling2D
...
def conv2d_bn(x,
              ...
              name=None):
    if name is not None:
        bn_name_base = name + "_bn"
        conv_name_base = name + '_conv'
    else:
        bn_name_base = None
        conv_name_base = None
    ...
    x = BatchNormalization(scale=False, name=bn_name_base)(x)
...
def InceptionV3(input_shape=[299, 299, 3], classes=1000):
    ...
    x = Dense(classes, activation='softmax', name='predicttions')(x)
    inputs = img_input
